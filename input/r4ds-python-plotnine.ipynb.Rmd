...
related:
  ...
  items:
  - type: post
    id: heuristics-for-translating-ggplot2-to-plotnine
  - type: workshop
    id: professional-data-visualisation-with-r
---
...
The section numbers in this tutorial link back to the corresponding section of the original text, in case you want to compare them.[^](It's important to note that this tutorial is not meant to compare Python and R. The never-ending flame wars between these two languages are boring and unproductive.) Only this preface and the few footnotes scattered among the text are entirely mine.
...
This tutorial focusses on plotnine. We'll also use a little numpy and pandas for data manipulation. To access the datasets, help pages, and functions that we will use in this tutorial, import[^](While it's generally to be considered bad practice to import everything into the global namespace, I think it's fine to do this in an ad-hoc environment such as a notebook as it makes using the many functions plotnine provides more convenient. An additional advantage is that the resulting code more closely resembles the original ggplot2 code. Alternatively, it's quite common to `import plotnine as p9` and prefix every function with `p9.`.) the necessary packages by running this code:
...
To plot `mpg`, run this code[^](If you dislike the continuation character `\` then an alternative syntax is to wrap the entire expression in parentheses so that it's not needed.) to put `displ` on the x-axis and `hwy` on the y-axis:
...
### [3.3.1](https://r4ds.had.co.nz/data-visualisation.html#exercises-1)&nbsp;&nbsp;&nbsp;Exercises[^](The original text has an additional exercise that contains code which is semantically wrong on purpose, but in plotnine, the corresponding code is also syntactically wrong. The reason is that in plotnine, you can only use column names in the aesthetic mapping and not literal values, e.g., `aes(color="blue")`.)
...
<div class="mx-auto w-5/6 md:w-1/2 px-4">
```{python, echo=FALSE, out.extra = ""}
ggplot(data=mpg, mapping=aes(x="displ", y="hwy")) +\
geom_point() +\
geom_smooth(se=False)
...
<!-- START_HIDE_IPYNB -->
    ...
    <div class="flex flex-wrap md:flex-row mb-4">
    <div class="mx-auto md:w-1/2 px-4 md:pl-8 md:pr-16">
    ```{python, echo=FALSE, out.extra = ""}
    ggplot(data=mpg, mapping=aes(x="class", y="hwy")) +\
    geom_boxplot()
    ```
    </div>
    <div class="mx-auto md:w-1/2 px-4 md:pr-8 md:pl-0">
    ```{python, echo=FALSE, out.extra = ""}
    ggplot(data=mpg, mapping=aes(x="class", y="hwy")) +\
    geom_boxplot() +\
    coord_flip()
...
for width in [4, 6, 8]:
    filename = f"{temp_dir}/save-width-{width}.png"
    plot.save(filename, width=width, height=width*0.618, dpi=300, verbose=False)
