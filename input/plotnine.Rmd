...
This chapter will teach you how to visualise your data using plotnine. Python has several systems for making graphs, but plotnine is one of the most elegant and most versatile. plotnine implements the __grammar of graphics__, a coherent system for describing and building graphs. With plotnine, you can do more faster by learning one system and applying it in many places.
...
<!-- TODO: remove mentions of tidyverse -->
This chapter focusses on plotnine, one of the core members of the tidyverse. To access the datasets, help pages, and functions that we will use in this chapter, load the tidyverse by running this code:
...
That one line of code loads the core tidyverse; packages which you will use in almost every data analysis. It also tells you which functions from the tidyverse conflict with functions in base Python (or from other packages you might have loaded). 

<!-- TODO: replace "tidyverse" and library -->
...
If we need to be explicit about where a function (or dataset) comes from, we'll use the special form `package::function()`. For example, `plotnine::ggplot()` tells you explicitly that we're using the `ggplot()` function from the plotnine package.
...
### [3.2.2](https://r4ds.had.co.nz/data-visualisation.html#creating-a-ggplot)&nbsp;&nbsp;&nbsp;3.2.2 Creating a ggplot
...
<!-- TODO: Include png directly from r4ds and add (foot)note -->
